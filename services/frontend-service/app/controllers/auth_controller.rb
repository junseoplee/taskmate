class AuthController < ApplicationController
  ...
  def create
    ...
    if response.success?
      ...
      Rails.logger.info "=== Login Session Debug ==="
      Rails.logger.info "Session token from API: #{session_token}"
      Rails.logger.info "Session before login: #{session.inspect}"

      login_user(session_token)

      Rails.logger.info "Session after login: #{session.inspect}"
      Rails.logger.info "Session[:session_token] = #{session[:session_token]}"
    ...
    respond_to do |format|
      ...
      format.json {
        render json: {
          success: false,
          error: "Service connection error: #{e.message}"
